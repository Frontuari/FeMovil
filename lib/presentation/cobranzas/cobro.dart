...
class _CobroState extends State<Cobro> {
                          ...
                          children: [
                                              ...
                                              children: [
                                                ...
                                                TextSpan(
                                                  text: orderData[
                                                              'saldo_total'] !=
                                                          '{@nil: true}'
                                                      ? '\$${orderData['saldo_total'].toString()}'
                                                      : '',
                                                  style: const TextStyle(
                                                      fontFamily:
                                                          'Poppins Regular'),
                            ...
                            orderData['documentno_factura'] != null
                                ? CustomTextInfo(
                                    ...
                                    value: orderData['documentno_factura'].toString() !=""
                              ...
                              if (snapshot.connectionState == ConnectionState.waiting) {
                                ...
                                return Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      // Etiqueta superior opcional (si quieres que se vea fuera)
                                      // const Text("Cuenta Bancaria", style: TextStyle(fontFamily: 'Poppins Bold', fontSize: 14)),
                                      
                                      // Contenedor decorativo (Sombra y Bordes Redondeados)
                                      Container(
                                        decoration: BoxDecoration(
                                          color: Colors.white,
                                          borderRadius: BorderRadius.circular(15),
                                          boxShadow: [
                                            BoxShadow(
                                              ...
                                              blurRadius: 7,
                                              spreadRadius: 2,
                                              offset: const Offset(0, 3), // Sombra ligeramente desplazada hacia abajo
                                            )
                                          ],
                                        ),
                                        child: DropdownSearch<Map<String, dynamic>>(
                                          items: bankAccountsList,
                                          validator: (Map<String, dynamic>? item) {
                                            if (item == null) {
                                          ...
                                          autoValidateMode: AutovalidateMode.onUserInteraction,
                                          // Cómo se ve el ítem seleccionado
                                          itemAsString: (Map<String, dynamic> item) {
                                            if (item['bank_name'] == null) return "";
                                            return "${item['bank_name']} - ${item['account_last_4']} (${item['iso_code']})";
                                          },
                                          dropdownDecoratorProps: DropDownDecoratorProps(
                                                      dropdownSearchDecoration: InputDecoration(
                                                        labelText: "Cuenta Bancaria",
                                                        hintText: "Seleccione el banco destino",
                                                        labelStyle: const TextStyle(fontFamily: 'Poppins Regular', color: Colors.grey),
                                                        hintStyle: const TextStyle(fontFamily: 'Poppins Regular', color: Colors.grey),
                                                        prefixIcon: const Icon(Icons.account_balance, color: Colors.blueGrey), // Color de icono más suave
                                                        border: InputBorder.none,  
                                                        contentPadding: const EdgeInsets.symmetric(horizontal: 5, vertical: 0),
                                                      ),
                                                    ),

                                                    // Configuración del Menú Desplegable (Popup)
                                                    popupProps: PopupProps.menu(
                                                      ...
                                                      // Personalización de los items dentro de la lista
                                                      itemBuilder: (context, item, isSelected) {
                                                        return Container(
                                                          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15),
                                                          child: Text(
                                                            "${item['bank_name']} - ${item['account_last_4']}",
                                                            style: TextStyle(
                                                    ...
                                                    onChanged: (Map<String, dynamic>? newValue) {
                                                      ...
                                                      // Tu lógica de setState...
                                                      setState(() {
                                                        _selectsBankAccountId = newValue['c_bank_account_id'] ?? 0;
                                                        _bankAccountText = newValue['bank_name'] ?? "";
                                                        _selectCurrencyId = newValue['c_currency_id'] ?? 0;
                                                        _currencyText = newValue['iso_code'] ?? "";
                                                      });
                                                    },
                                                  ),
                                                ),
                               ...
                               ElevatedButton(
                              // Condiciones para habilitar el botón (visual)
                              onPressed: orderData['status_sincronized'] == 'Enviado' &&
                                  ...
                                  : null, // Si las condiciones iniciales no se cumplen, el botón está deshabilitado
  ...
  Future<void> _createCobro() async {
    ...
    if (payAmt > saldoTotal) {
      ...
      ErrorMessage.showErrorMessageDialog(context, 'El cobro no puede ser mayor al saldo total de la orden.');
      return;
    } else if (payAmt <= 0 || saldoTotal <= 0) {
      ErrorMessage.showErrorMessageDialog(context, 'El cobro no puede ser menor o igual a 0.');
      return;
    } else {
     ...
     showLoadingDialog(context, message: 'Enviando cobro...');  
      dynamic response = await createCobroIdempiere(cobro);



      dynamic numDoc = findValueByColumn(response, 'DocumentNo') ?? 0;
      dynamic cPaymentID = findValueByColumn(response, 'C_Payment_ID') ?? 0;
      final isErrorTrue = findIsError(response) ?? false;
      ...
      dynamic errorMessage = findErrorMessage(response) ?? 'Error desconocido'  ;
        ...
        SuccesMessages.showSuccesMessagesDialog(
            context, 'Cobro creado con éxito. Número de Documento: $numDoc',
            goBack: true);
      }
