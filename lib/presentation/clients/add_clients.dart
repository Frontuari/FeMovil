          ...
          if (ifExists) {
            showDialog(
                ...
                builder: (BuildContext context) {
                  return AlertDialog(
                    title: Text('Cliente ya registrado',
                        style: TextStyle(
                  ...
                  children: <Widget>[
                    ...
                    CustomDropdownButtonFormField(
                      ...
                      onSelected: (newValue, newText) async {
                        ...
                        newCityList.add(
                            {'c_city_id': 0, 'city': 'Selecciona una ciudad'});
                        if (countryName != "") {
    ...
    if (isErrorTrue) {
      ...
      if (c_BPartner_ID == 0) {
          await WarningClientModal.show(
            context,
            title: 'Advertencia',
            message: 'Se creó el cliente a nivel de APP.\n\nPor favor, Sincronice en Clientes. \n\n¿Qué Deseas Hacer?',goBack: true,
            stayButton: true
          );       
        await saveClientToDatabase(clientResponse);
      } else {
        print("TERCERO CREADO EXITOSAMENTE: Traer Data $c_BPartner_ID");
        await saveClientToDatabase(clientResponse);
      dynamic clientData =await getClientbpartnerIDData(c_BPartner_ID);

          print("DATA DEL TERCERO CREADO EXITOSAMENTE: $clientData");

         SuccessClientModal.show(context, clientData,
          goBack: true,
        stayButton: true
        );
      }
