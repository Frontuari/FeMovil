...
class _AddProvidersFormState extends State<AddProvidersForm> {
  ...
  loadList() async {
    ...
    _countryVendorList
        .add({'c_country_id': 0, 'country': 'Selecciona un país'});
    _provinceList.add({'c_region_id': 0, 'region': 'Selecciona una provincia'});
    _cityList.add({'c_city_id': 0, 'city': 'Selecciona una ciudad'});
    ...
    if (getCountryVendor.isNotEmpty) {
      ...
      List<Map<String, dynamic>> provinceListByCountry =
          await listarRegions(defaultCountry);
          ...
          if (ifExists) {
            showDialog(
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  title: Text('Proveedor ya registrado',
                      style: TextStyle(
                          fontFamily: 'Poppins SemiBold', fontSize: 18)),
                  content: Text(
                      'El proveedor con el ID fiscal \'$rucText\' ya está registrado en el sistema.',
                      style: TextStyle(
                    ...
                    children: <Widget>[
                      ...
                      CustomDropdownButtonFormFieldVendor(
                        ...
                        onSelected: (newValue, idTypeText) {
                          setState(() {
                            ...
                            if (idTypeText == 'C CEDULA' ||
                                idTypeText == 'P PASAPORTE') {
                              _idMaxlength = 10;
                            } else if (idTypeText == 'R RUC PERSONAL' ||
                                idTypeText == 'R RUC JURIDICO') {
                              _idMaxlength = 13;
                            }
                        ...
                        child: TextFormField(
                          ...
                          validator: (value) {
                            if (value == null || value.isEmpty) {
                              ...
                              if (_idTypeText == 'P PASAPORTE' &&
                                  value.length < _idMaxlength) {
                                return 'Debe contener al menos $_idMaxlength dígitos';
                              }

                              // VERIFICA SI EL IDENTIFICADOR ES CEDULA O RUC
                              if (_idTypeText == 'C CEDULA' ||
                                  _idTypeText == 'R RUC PERSONAL' ||
                                  _idTypeText == 'R RUC JURIDICO') {
                                try {
                      ...
                      CustomDropdownButtonFormFieldVendor(
                        ...
                        onSelected: (newValue, newText) async {
                          ...
                          if (countryName != "") {
                            dynamic provinceListByCountry =
                                await listarRegions(countryIndex);
                      ...
                      CustomDropdownButtonFormFieldVendor(
                        ...
                        onSelected: (newValue, newText) async {
                          ...
                          if (provinceName != "") {
                            dynamic citiesListByRegion =
                                await listarCities(provinceIndex);
  ...
  void _saveProduct() async {
    ...
    final responseIdempiere = await createVendorIdempiere(provider.toMap());
    print('Respuesta de iDempiere: $responseIdempiere');

    final c_BPartner_ID =
        findValueByColumn(responseIdempiere, 'C_BPartner_ID') ?? 0;
    final c_Location_ID =
        findValueByColumn(responseIdempiere, 'C_Location_ID') ?? 0;
    final c_BPartner_Location_ID =
        findValueByColumn(responseIdempiere, 'C_BPartner_Location_ID') ?? 0;

    final isErrorTrue = findIsError(responseIdempiere) ?? false;
    print(
        'C_BPartner_ID: $c_BPartner_ID, C_Location_ID: $c_Location_ID, C_BPartner_Location_ID: $c_BPartner_Location_ID is ERROR $isErrorTrue');

    Vendor responseProvider = Vendor(
      ...
      lcoTaxIdTypeId: taxTypeId,
      taxIdTypeName: taxIdText,
      cBPartnerLocationId: c_Location_ID,
      isBillTo: 'Y',
      phone: phone,
      cLocationId: c_BPartner_Location_ID,
      address: address,
      city: city,
      countryName: countryName,
      postal: codePostal,
    ...
    if (isErrorTrue) {
      ...
      if (c_BPartner_ID == 0) {
        WarningMessages.showWarningMessagesDialog(
            context, 'Se Creo cliente a Nivel de APP pero No en el sistema',
            goBack: true);
        ...
        SuccesMessages.showSuccesMessagesDialog(
            context, 'Proveedor Creado a Nivel Local \nN° RUC:$taxId',
            goBack: true);
        await saveProviderToDatabase(provider);
      }
    }
